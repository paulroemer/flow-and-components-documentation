...
image:images/dom-to-java.svg[dom-to-java,1000,300]
...
See https://vaadin.com/components/browse[Components] for the full set of available Vaadin components.
...
See <<../creating-components/tutorial-component-basic#,Creating Components>> to learn various ways to build components with a reusable API, and <<../element-api/tutorial-event-listener#,Element API>> to learn how to access and customize the DOM from the server side.
...
See <<../web-components/integrating-a-web-component#,Integrating a Web Component>> for more.
...
Another way to create components is to separate the layout from the UI logic. The best way to do this is to use HTML templates together with Java classes. The HTML files contain the layout and pure client-side logic (if needed), while the Java classes take care of server-side logic, like event handling.
...
See <<../polymer-templates/tutorial-template-basic#,Creating Polymer Templates>> for more.


== Routing and Navigation

Vaadin Flow provides the `Router` class to structure the navigation of your web app or website into logical parts.
...
See <<../routing/tutorial-routing-annotation#,Routing and Navigation>> for more.


== How Vaadin Flow Components Work

Vaadin Flow allows Java code to control the DOM in the web browser, by providing a server-side Java representation of the same DOM tree.
